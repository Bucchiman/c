...
void XWDView(Display *d, char name[], imElement *im)
{
  ...
  flg=fread(buff,fh.bits_per_pixel/8,fh.window_width*fh.window_height,fp);
  fclose(fp);

  im->width=fh.window_width;
  im->height=fh.window_height;

  im->im=XCreateImage(d,DefaultVisual(d,0),DefaultDepth(d,0),ZPixmap,0,0,im->width,im->height,32,0);
